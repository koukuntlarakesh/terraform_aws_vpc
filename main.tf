...
resource "aws_nat_gateway" "main" {
  allocation_id = aws_eip.eip.id
   subnet_id =  aws_subnet.public[0].id #assuming first public is the one for NAT gateway
   tags = merge(var.common_tags, var.roboshop_nat_gateway_tags,
   {
...
resource "aws_route_table" "public" {
  vpc_id = aws_vpc.main.id
  tags = merge(var.common_tags,var.public_route_tables_tags,
  {
...
resource "aws_route_table" "private" {
  ...
  vpc_id = aws_vpc.main.id
  tags = merge(var.common_tags,var.private_route_tables_tags,
  {
...
resource "aws_route_table" "database" {
  ...
  vpc_id = aws_vpc.main.id
  tags = merge(var.common_tags,var.database_route_tables_tags,
  {
